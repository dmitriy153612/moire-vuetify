...
const updatedSelectedColorId = computed<number>({
  get: () => {
    return props.colorOptions.findIndex((option) => option.id === props.selectedColorId)
  },
  set: (newValue) => {
...
const imgUrl = computed<string>(() => {
  const selectedColorOption = props.colorOptions.find((item) => item.id === props.selectedColorId)
  return selectedColorOption?.imgUrl || ''
})
</script>
